...
Enhanced version that validates coordinates against real-world features
like buildings, sports fields, and water bodies using OpenStreetMap data.

WARNING: This is SLOW due to API rate limiting (0.5s delay per coordinate).
For 85 trees, expect ~45 seconds of validation time.

Only use this when you want maximum accuracy and are willing to wait.
"""

import sys

# Set validate_landuse flag
VALIDATE_LANDUSE = True

print("=" * 70)
print("LANDUSE VALIDATION ENABLED")
print("=" * 70)
print("This will check each coordinate against OpenStreetMap to avoid:")
print("  - Buildings")
print("  - Sports fields/courts")
print("  - Water bodies")
print("  - Paved areas (parking lots, roads)")
print("  - Playgrounds")
print()
print("WARNING: This is SLOW (~0.5s per tree)")
print(f"Expected time: ~45 seconds for 85 trees")
print("=" * 70)
print()

response = input("Continue with landuse validation? (yes/no): ")
if response.lower() not in ['yes', 'y']:
    print("Aborted. Use 'python3 build_trees_with_accuracy.py' for faster build without landuse validation.")
    sys.exit(0)

# Now run the build script with landuse validation
import pandas as pd
import json
...
for _, row in df_locations.iterrows():
    ...
    trees_by_location[key] = 0

# Count trees per location
...
# Generate dispersed coordinates WITH LANDUSE VALIDATION
print("\nGenerating dispersed coordinates with landuse validation...")
print("(This will be slow due to API rate limiting)")
location_coords = {}
...
for loc_key, num_trees in trees_by_location.items():
    if loc_key in location_lookup and num_trees > 0:
        ...
        print(f"\n  {park_name} > {loc_key[1]}: {num_trees} trees")
        ...
        coords_list = generate_dispersed_coordinates(
            ...
            radius_meters=50,
            park_name=park_name,
            validate_landuse=True  # ENABLE LANDUSE VALIDATION
        )
        location_coords[loc_key] = coords_list

        valid_count = sum(1 for lat, lon in coords_list if is_point_in_park(lat, lon, park_name)[0])
        print(f"  ✓ Generated: {valid_count}/{num_trees} within park bounds")

# Build trees (same as build_trees_with_accuracy.py)
print("\nParsing survey data with validated coordinates...")
trees = []
current_park = None
current_area = None
tree_counter = 1
...
print(f"\nExtracted {len(trees)} trees with landuse-validated coordinates")
...
geojson = {
    ...
    "metadata": {
        ...
        "notes": "Tree locations validated against park boundaries AND real-world features (buildings, sports fields, water bodies) using OpenStreetMap data.",
        "accuracy_levels": {
            "confirmed": "GPS-verified location (±5m)",
            "area": "Geocoded to specific park area, validated against buildings/water (±50m)",
